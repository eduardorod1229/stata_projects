...
quietly infix                       ///
  ...
  double  strata           52-63    ///
  byte    gq               64-64    ///
  str     us2000a_state    65-66    ///
  str     us2000a_region   67-67    ///
  int     pernum           68-71    ///
  double  perwt            72-81    ///
  byte    sex              82-82    ///
  int     age              83-85    ///
  ...
  byte    migtyp00         102-103  ///
  byte    disabwrk         104-104  ///
  byte    diffrem          105-105  ///
  byte    diffphys         106-106  ///
  byte    diffmob          107-107  ///
  byte    diffcare         108-108  ///
  byte    diffsens         109-109  ///
  byte    pwtype           110-110  ///
  str     us2000a_pweight  111-114  ///
  str     us2000a_relate   115-116  ///
  str     us2000a_sex      117-117  ///
  str     us2000a_ageorig  118-119  ///
  str     us2000a_hispan   120-121  ///
  str     us2000a_numrace  122-122  ///
  str     us2000a_white    123-123  ///
  str     us2000a_black    124-124  ///
  str     us2000a_aian     125-125  ///
  str     us2000a_asian    126-126  ///
  str     us2000a_nhpi     127-127  ///
  str     us2000a_other    128-128  ///
  str     us2000a_marstat  129-129  ///
  str     us2000a_msp      130-130  ///
  str     us2000a_sfn      131-131  ///
  str     us2000a_educ     132-133  ///
  str     us2000a_mob      134-134  ///
  str     us2000a_sensry   135-135  ///
  str     us2000a_physcl   136-136  ///
  str     us2000a_mental   137-137  ///
  str     us2000a_slfcare  138-138  ///
  str     us2000a_abgo     139-139  ///
  str     us2000a_abwork   140-140  ///
  str     us2000a_disable  141-141  ///
  str     us2000a_esr      142-142  ///
  str     us2000a_incws    143-148  ///
  str     us2000a_incse    149-154  ///
  str     us2000a_povrty   155-157  ///
  str     us2000a_age      158-159  ///
  using `"usa_00009.dat"'
...
keep if age >=18
recode diffmob (1=0 "No independent living difficulty") (2=1 "Has independent living difficulty"), gen(diff_mob)
generate c_age = (age - 45)/10
gen low_educ=1 if educ <=2
...
keep if year==1990
by state, sort: egen state_low_educ= mean(100 * low_educ)


save data_1990.dta, replace

use data_combined.dta, clear


keep if year==2000

save data_2000.dta, replace

/// Extract data from the 1990 survey


use data_1990.dta, clear
...
save dataset_part2_1990.dta, replace


mixed diff_mob c_age || statefip:


///Extract data from the 2000 survey 

Manipulating 2000 survey

use data_2000.dta, clear
set seed 44 
sample 10
gen sampl_weight = perwt * 10
by state, sort: egen state_low_educ= mean(100 * low_educ)
save dataset_2000_part2, replace
...
///Model 1
regress diff_mob low_educ c_age female state_low_educ

svyset cluster [pweight=sampl_weight], strata(strata)

svy: regress diff_mob low_educ c_age female state_low_educ


// Design effect
di 0.0017973^2/0.001085^2


///Model 2

mixed diff_mob || statefip:


//Model 3
mixed diff_mob state_low_educ || statefip: 


//Model 4
mixed diff_mob state_low_educ low_educ c_age female || statefip: 


//Model 5
gen educ_interaction = low_educ*state_low_educ 

mixed diff_mob state_low_educ low_educ c_age female educ_interaction || statefip: 


//Model 4.m
mixed diff_mob state_low_educ low_educ educ c_age female || statefip:
...
use dataset_part2_1990.dta, clear

mixed diff_mob c_age || statefip:

///Pooling data

append using dataset_2000_part2.dta
save pooled_data.dta, replace
...
mixed diff_mob c.c_age##i.year || statefip:
...
use dataset_2000_part2, clear

gen birth_yr = year-age

recode birth_yr (min/1909 = 1 "1900") (1910/1919 = 2 "1910") (1920/1929=3 "1920") (1930/1939=4 "1930") (1940/1949=5 "1940") (1950/1959=6 "1950") (1960/1969=7 "1960") (1970/1979=8 "1970") (1980/1989=9 "1980"), gen(decenial)

label define marstat 1"Now married" 2"Widowed" 3"Divorced" 4"Separated" 5"Never married (includes under 15 years)

destring us2000a_marstat, generate(marstat)

label values marstat marstat

recode  disabwrk (1/2=0 "No disability that affects work") (3/4=1 "Has disability to work"), gen(work_disab)

///Poverty level: The higher the number the person is ABOVE poverty level
destring us2000a_povrty, gen(povrty)

save dataset_part3.dta, replace

keep if birth_yr <1980

logistic work_disab i.educ i. marstat i.decenial 


/// Decenial 1900

gen per_urb_birth_yr = 11.9 if decenial==1 & statefip==1 //Alabama
replace per_urb_birth_yr = 24.5 if decenial==1 & statefip==2 //Alaska
replace per_urb_birth_yr = 15.90 if decenial==1 & statefip==4 //Arizona
replace per_urb_birth_yr = 8.5 if decenial==1 & statefip==5 //Arkansas
replace per_urb_birth_yr = 52.3 if decenial==1 & statefip==6 //California
replace per_urb_birth_yr = 48.3 if decenial==1 & statefip==8 //Colorado
replace per_urb_birth_yr = 59.9 if decenial==1 & statefip==9 //Connecticut
replace per_urb_birth_yr = 46.4 if decenial==1 & statefip==10 //Delaware
replace per_urb_birth_yr = 100.0 if decenial==1 & statefip==11 //D.C
replace per_urb_birth_yr = 20.3 if decenial==1 & statefip==12 //Florida
replace per_urb_birth_yr = 15.6 if decenial==1 & statefip==13 //Georgia
replace per_urb_birth_yr = 25.5 if decenial==1 & statefip==15 //Hawaii
replace per_urb_birth_yr = 6.2 if decenial==1 & statefip==16 //Idaho
replace per_urb_birth_yr = 54.3 if decenial==1 & statefip==17 //Illinois
replace per_urb_birth_yr = 34.30 if decenial==1 & statefip==18 //Indiana
replace per_urb_birth_yr = 25.60 if decenial==1 & statefip==19 //Iowa
replace per_urb_birth_yr = 22.40 if decenial==1 & statefip==20 //Kansas
replace per_urb_birth_yr = 21.80 if decenial==1 & statefip==21 //Kentucky
replace per_urb_birth_yr = 26.50 if decenial==1 & statefip==22 //Lousiana
replace per_urb_birth_yr = 33.50 if decenial==1 & statefip==23 //Maine
replace per_urb_birth_yr = 49.80 if decenial==1 & statefip==24 //Maryland
replace per_urb_birth_yr = 86.00 if decenial==1 & statefip==25 //Massachusetts
replace per_urb_birth_yr = 39.30 if decenial==1 & statefip==26 //Michigan
replace per_urb_birth_yr = 34.10 if decenial==1 & statefip==27 //Minnesota
replace per_urb_birth_yr = 7.70 if decenial==1 & statefip==28 //Mississippi
replace per_urb_birth_yr = 36.30 if decenial==1 & statefip==29 //Missouri
replace per_urb_birth_yr = 34.70 if decenial==1 & statefip==30 //Montana
replace per_urb_birth_yr = 23.70 if decenial==1 & statefip==31 //Nebraska
replace per_urb_birth_yr = 17.00 if decenial==1 & statefip==32 //Nevada
replace per_urb_birth_yr = 46.70 if decenial==1 & statefip==33 //New Hampshire
replace per_urb_birth_yr = 70.60 if decenial==1 & statefip==34 //New Jersey
replace per_urb_birth_yr = 14.00 if decenial==1 & statefip==35 //New Mexico
replace per_urb_birth_yr = 72.90 if decenial==1 & statefip==36 //New York
replace per_urb_birth_yr = 9.90 if decenial==1 & statefip==37 //North Carolina
replace per_urb_birth_yr = 7.30 if decenial==1 & statefip==38 //North Dakota
replace per_urb_birth_yr = 48.10 if decenial==1 & statefip==39 //Ohio
replace per_urb_birth_yr = 7.40 if decenial==1 & statefip==40 //Oklahoma
replace per_urb_birth_yr = 32.20 if decenial==1 & statefip==41 //Oregon
replace per_urb_birth_yr = 54.70 if decenial==1 & statefip==42 //Pennsylvania
replace per_urb_birth_yr = 88.30 if decenial==1 & statefip==44 //Rhode Island
replace per_urb_birth_yr = 12.80 if decenial==1 & statefip==45 //South Carolina
replace per_urb_birth_yr = 10.20 if decenial==1 & statefip==46 //South Dakota
replace per_urb_birth_yr = 16.20 if decenial==1 & statefip==47 //Tennessee
replace per_urb_birth_yr = 17.10 if decenial==1 & statefip==48 //Texas
replace per_urb_birth_yr = 38.10 if decenial==1 & statefip==49 //Utah
replace per_urb_birth_yr = 22.10 if decenial==1 & statefip==50 //Vermont
replace per_urb_birth_yr = 18.30 if decenial==1 & statefip==51 //Virginia
replace per_urb_birth_yr = 40.80 if decenial==1 & statefip==53 //Washington
replace per_urb_birth_yr = 13.10 if decenial==1 & statefip==54 //West Virginia
replace per_urb_birth_yr = 38.20 if decenial==1 & statefip==55 //Wisconsin
replace per_urb_birth_yr = 28.80 if decenial==1 & statefip==56 //Wyoming


/// Decenial 1910

replace per_urb_birth_yr = 17.30 if decenial==2 & statefip==1 //Alabama
replace per_urb_birth_yr = 9.50 if decenial==2 & statefip==2 //Alaska
replace per_urb_birth_yr = 31.0 if decenial==2 & statefip==4 //Arizona
replace per_urb_birth_yr = 12.90 if decenial==2 & statefip==5 //Arkansas
replace per_urb_birth_yr = 61.80 if decenial==2 & statefip==6 //California
replace per_urb_birth_yr = 50.30 if decenial==2 & statefip==8 //Colorado
replace per_urb_birth_yr = 65.60 if decenial==2 & statefip==9 //Connecticut
replace per_urb_birth_yr = 48.00 if decenial==2 & statefip==10 //Delaware
replace per_urb_birth_yr = 100.0 if decenial==2 & statefip==11 //D.C
replace per_urb_birth_yr = 29.10 if decenial==2 & statefip==12 //Florida
replace per_urb_birth_yr = 20.60 if decenial==2 & statefip==13 //Georgia
replace per_urb_birth_yr = 30.70 if decenial==2 & statefip==15 //Hawaii
replace per_urb_birth_yr = 21.50 if decenial==2 & statefip==16 //Idaho
replace per_urb_birth_yr = 61.70 if decenial==2 & statefip==17 //Illinois
replace per_urb_birth_yr = 42.40 if decenial==2 & statefip==18 //Indiana
replace per_urb_birth_yr = 30.60 if decenial==2 & statefip==19 //Iowa
replace per_urb_birth_yr = 29.10 if decenial==2 & statefip==20 //Kansas
replace per_urb_birth_yr = 24.30 if decenial==2 & statefip==21 //Kentucky
replace per_urb_birth_yr = 30.00 if decenial==2 & statefip==22 //Lousiana
replace per_urb_birth_yr = 35.30 if decenial==2 & statefip==23 //Maine
replace per_urb_birth_yr = 50.80 if decenial==2 & statefip==24 //Maryland
replace per_urb_birth_yr = 89.00 if decenial==2 & statefip==25 //Massachusetts
replace per_urb_birth_yr = 47.20 if decenial==2 & statefip==26 //Michigan
replace per_urb_birth_yr = 41.00 if decenial==2 & statefip==27 //Minnesota
replace per_urb_birth_yr = 11.50 if decenial==2 & statefip==28 //Mississippi
replace per_urb_birth_yr = 42.30 if decenial==2 & statefip==29 //Missouri
replace per_urb_birth_yr = 35.50 if decenial==2 & statefip==30 //Montana
replace per_urb_birth_yr = 26.10 if decenial==2 & statefip==31 //Nebraska
replace per_urb_birth_yr = 16.30 if decenial==2 & statefip==32 //Nevada
replace per_urb_birth_yr = 51.80 if decenial==2 & statefip==33 //New Hampshire
replace per_urb_birth_yr = 51.80 if decenial==2 & statefip==34 //New Jersey
replace per_urb_birth_yr = 14.20 if decenial==2 & statefip==35 //New Mexico
replace per_urb_birth_yr = 78.90 if decenial==2 & statefip==36 //New York
replace per_urb_birth_yr = 14.40 if decenial==2 & statefip==37 //North Carolina
replace per_urb_birth_yr = 11.00 if decenial==2 & statefip==38 //North Dakota
replace per_urb_birth_yr = 55.90 if decenial==2 & statefip==39 //Ohio
replace per_urb_birth_yr = 19.20 if decenial==2 & statefip==40 //Oklahoma
replace per_urb_birth_yr = 45.60 if decenial==2 & statefip==41 //Oregon
replace per_urb_birth_yr = 60.40 if decenial==2 & statefip==42 //Pennsylvania
replace per_urb_birth_yr = 91.00 if decenial==2 & statefip==44 //Rhode Island
replace per_urb_birth_yr = 14.80 if decenial==2 & statefip==45 //South Carolina
replace per_urb_birth_yr = 13.10 if decenial==2 & statefip==46 //South Dakota
replace per_urb_birth_yr = 20.20 if decenial==2 & statefip==47 //Tennessee
replace per_urb_birth_yr = 24.10 if decenial==2 & statefip==48 //Texas
replace per_urb_birth_yr = 46.30 if decenial==2 & statefip==49 //Utah
replace per_urb_birth_yr = 27.80 if decenial==2 & statefip==50 //Vermont
replace per_urb_birth_yr = 23.10 if decenial==2 & statefip==51 //Virginia
replace per_urb_birth_yr = 53.00 if decenial==2 & statefip==53 //Washington
replace per_urb_birth_yr = 18.70 if decenial==2 & statefip==54 //West Virginia
replace per_urb_birth_yr = 43.00 if decenial==2 & statefip==55 //Wisconsin
